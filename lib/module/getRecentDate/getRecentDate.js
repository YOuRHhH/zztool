 ...
 * })
 */
export function getRecentDate(options) {
    let { date = new Date(), step = 1, type = 'after', format = 'Y-M-D', option = { 1: 3, 2: 7, 3: 15, 4: 30 } } = options;
    const oneDay = 86400000;
    let now = new Date(date).getTime();
    if (isNaN(now))
        return [];
    const optionKeys = Object.keys(option);
    if (optionKeys.includes(step.toString()) === false)
        step = Number(optionKeys[0]);
