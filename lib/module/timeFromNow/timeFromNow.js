...
export function timeFromNow(timestamp, option) {
    ...
    const diff = timestamp - +Date.now(), seconds = Math.floor(diff / 1000), minutes = Math.floor(seconds / 60), hours = Math.floor(minutes / 60), days = Math.floor(hours / 24), months = Math.floor(days / 30), years = Math.floor(days / 365);
