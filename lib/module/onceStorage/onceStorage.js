 ...
 */
export function onceStorage(key, value) {
    try {
 ...
 * @returns
 */
export function getOnceStorage(key) {
 ...
 */
export function getAllOnceStorage() {
    const onceStorageKeys = Object.keys(localStorage).filter(item => item.startsWith(keyStr));
    if (!onceStorageKeys.length)
        return [];
    return onceStorageKeys.map((keyIndex) => {
 ...
 * @returns
 */
export function removeOnceStorage(key) {
    try {
